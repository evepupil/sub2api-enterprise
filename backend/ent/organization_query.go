...
func (_q *OrganizationQuery) QueryInvitations() *RedeemCodeQuery {
...
func (_q *OrganizationQuery) WithInvitations(opts ...func(*RedeemCodeQuery)) *OrganizationQuery {
	query := (&RedeemCodeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) loadInvitations(ctx context.Context, query *RedeemCodeQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *RedeemCode)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(redeemcode.FieldOrganizationID)
	}
...
func (_q *OrganizationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withOwner != nil {
			_spec.Node.AddColumnOnce(organization.FieldOwnerUserID)
		}
	}
