...
// was not loaded in eager-loading, or loaded but was not found.
func (e RedeemCodeEdges) OrganizationOrErr() (*Organization, error) {
...
func (*RedeemCode) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case redeemcode.FieldID, redeemcode.FieldUsedBy, redeemcode.FieldGroupID, redeemcode.FieldOrganizationID, redeemcode.FieldValidityDays:
...
func (_m *RedeemCode) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case redeemcode.FieldOrganizationID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
...
// QueryOrganization queries the "organization" edge of the RedeemCode entity.
func (_m *RedeemCode) QueryOrganization() *OrganizationQuery {
	return NewRedeemCodeClient(_m.config).QueryOrganization(_m)
}
