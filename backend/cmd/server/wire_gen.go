...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	organizationSpendingRepository := repository.NewOrganizationSpendingRepository(client)
	billingCacheService := service.ProvideBillingCacheService(billingCache, userRepository, userSubscriptionRepository, apiKeyRepository, userRPMCache, userGroupRateRepository, configConfig, serviceUserPlatformQuotaRepository, organizationSpendingRepository)
	apiKeyCache := repository.NewAPIKeyCache(redisClient)
	concurrencyCache := repository.ProvideConcurrencyCache(redisClient, configConfig)
	schedulerCache := repository.ProvideSchedulerCache(redisClient, configConfig)
	accountRepository := repository.NewAccountRepository(client, db, schedulerCache)
	concurrencyService := service.ProvideConcurrencyService(concurrencyCache, accountRepository, configConfig)
	organizationGroupRepository := repository.NewOrganizationGroupRepository(client)
	organizationGroupService := service.NewOrganizationGroupService(organizationGroupRepository)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, concurrencyService, organizationGroupService)
	...
	organizationRepository := repository.NewOrganizationRepository(client)
	organizationService := service.NewOrganizationService(organizationRepository, redeemCodeRepository)
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, tencentCaptchaService, aliyunCaptchaService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, organizationService)
	...
	organizationMemberRepository := repository.NewOrganizationMemberRepository(client)
	organizationMemberService := service.NewOrganizationMemberService(organizationService, organizationMemberRepository, userRepository, apiKeyAuthCacheInvalidator)
	usageHandler := handler.ProvideUsageHandler(usageService, apiKeyService, opsService, settingService, organizationMemberService)
	...
	adminOrganizationRepository := repository.NewAdminOrganizationRepository(client)
	adminOrganizationService := service.NewAdminOrganizationService(adminOrganizationRepository, organizationGroupRepository, groupRepository, apiKeyAuthCacheInvalidator)
	organizationHandler := admin.NewOrganizationHandler(adminOrganizationService)
	...
	publicStatusService := service.ProvidePublicStatusService(channelMonitorService, settingService)
	publicStatusHandler := handler.NewPublicStatusHandler(publicStatusService)
	...
	handlerOrganizationHandler := handler.NewOrganizationHandler(organizationService)
