<template>
  <DataTable :columns="columns" :data="orders" :loading="loading">
    ...
    <template #cell-out_trade_no="{ value }">
      <span class="text-sm text-content-strong">{{ value }}</span>
    </template>
    <template v-if="showUser" #cell-user_email="{ value, row }">
      <div class="text-sm">
        <span class="text-content-strong">{{ value || row.user_name || '#' + row.user_id }}</span>
        <span v-if="row.user_notes" class="ml-1 text-xs text-gray-400">({{ row.user_notes }})</span>
      </div>
    </template>
    <template #cell-pay_amount="{ value, row }">
      <div class="text-sm">
        <span class="font-medium text-content-strong">{{ paymentAmountSymbol(row) }}{{ value.toFixed(2) }}</span>
    ...
    <template #cell-payment_type="{ value }">
      <span class="text-sm text-content">{{ t('payment.methods.' + value, value) }}</span>
    </template>
    <template #cell-status="{ value }">
      <OrderStatusBadge :status="value" />
    </template>
    <template #cell-created_at="{ value }">
      <span class="text-xs text-content-muted">{{ formatDate(value) }}</span>
    </template>
