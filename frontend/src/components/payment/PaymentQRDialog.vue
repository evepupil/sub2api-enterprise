  ...
  <BaseDialog :show="show" :title="dialogTitle" width="narrow" @close="handleClose">
    ...
    <div v-if="!success" class="flex flex-col items-center space-y-4">
      ...
      <template v-if="qrUrl">
        ...
        <p v-if="scanHint" class="text-center text-sm text-content-muted">
          {{ scanHint }}
        </p>
      </template>
      ...
      <template v-else>
        <div class="flex flex-col items-center py-4">
          ...
          <p class="mt-4 text-sm text-content-muted">{{ t('payment.qr.payInNewWindowHint') }}</p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-content-muted">{{ qrUrl ? t('payment.qr.expiresIn') : '' }}</p>
    ...
    <div v-else class="flex flex-col items-center space-y-4 py-4">
      ...
      <p class="text-lg font-bold text-content-strong">{{ t('payment.result.success') }}</p>
      <div v-if="paidOrder" class="w-full rounded-xl bg-gray-50 p-4 dark:bg-dark-800">
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-medium text-content-strong">{{ paymentAmountSymbol(paidOrder) }}{{ paidOrder.pay_amount.toFixed(2) }}</span>
          </div>
        </div>
      </div>
    </div>
