    ...
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <div v-if="supportsPaymentMode" class="flex items-center gap-2">
          <span class="text-xs font-medium text-content-muted">{{ t('admin.settings.payment.paymentMode') }}</span>
        ...
        <div v-if="availableTypes.length > 1" class="flex items-center gap-2">
          <span class="text-xs font-medium text-content-muted">{{ t('admin.settings.payment.supportedTypes') }}</span>
      ...
      <div v-if="form.provider_key === 'easypay'" class="space-y-3 rounded-lg border border-gray-100 p-3 dark:border-dark-700">
        <div class="flex items-center justify-between gap-3">
          <div>
            <h5 class="text-sm font-medium text-content-strong">
        ...
        <div v-if="easyPayCustomMethods.length" class="space-y-2">
          ...
          >
            <div>
              <label class="text-xs text-content-muted">{{ t('admin.settings.payment.customMethodType') }}</label>
              <input v-model="method.type" type="text" class="input mt-0.5" placeholder="credit_card" />
            </div>
            <div>
              <label class="text-xs text-content-muted">{{ t('admin.settings.payment.customMethodUpstreamType') }}</label>
              <input v-model="method.upstreamType" type="text" class="input mt-0.5" placeholder="credit_card" />
            </div>
            <div>
              <label class="text-xs text-content-muted">{{ t('admin.settings.payment.customMethodDisplayName') }}</label>
      ...
      <div class="border-t border-gray-200 pt-4 dark:border-dark-700">
        <div class="mb-3 flex items-center gap-2">
          <h4 class="text-sm font-semibold text-content-strong">
        ...
        <p v-if="paymentGuide" class="mb-3 text-xs text-content-muted">
          ...
          <div v-for="field in resolvedFields" :key="field.key">
            ...
            <div v-else-if="field.sensitive" class="relative">
              ...
              <button
                ...
                class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-content"
              >
            ...
            />
            <p v-if="field.hintKey" class="mt-1 text-xs leading-relaxed text-content-muted">
              {{ t(field.hintKey) }}
            </p>
          </div>
        </div>
      ...
      <div v-if="limitableTypes.length" class="border-t border-gray-200 pt-4 dark:border-dark-700">
        ...
        <div v-show="limitsExpanded" class="mt-3 space-y-3">
          ...
          >
            <p class="mb-2 text-xs font-medium text-content">{{ lt.label }}</p>
            <div class="grid grid-cols-3 gap-3">
              <div>
                <label class="text-xs text-content-muted">{{ t('admin.settings.payment.limitSingleMin') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-content-muted">{{ t('admin.settings.payment.limitSingleMax') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-content-muted">{{ t('admin.settings.payment.limitDaily') }}</label>
                <input
                  type="number"
          ...
          <p class="text-xs text-content-subtle">{{ t('admin.settings.payment.limitsHint') }}</p>
        </div>
      </div>
    </form>
