    ...
    <div class="space-y-2">
      ...
      >
        <div class="flex flex-col">
          <span class="font-medium text-content-strong">{{ formatMonitorModel(r.model) }}</span>
          <span v-if="r.message" class="text-xs text-content-muted">{{ r.message }}</span>
        ...
        <div class="flex items-center gap-2">
          ...
          <span class="text-xs text-content-muted">{{ formatLatency(r.latency_ms) }} ms</span>
        </div>
      </div>
    </div>
    <template #footer>
