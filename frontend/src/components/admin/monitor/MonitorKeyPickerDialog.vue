      ...
      <div v-else class="max-h-96 overflow-auto rounded-lg border border-line-strong">
        <table class="w-full text-sm">
          <thead class="bg-surface-sunken sticky top-0 z-10">
            <tr class="text-left text-xs font-medium uppercase tracking-wider text-content-muted">
          ...
          <tbody class="divide-y divide-line-subtle">
            <tr
              ...
              :key="k.id"
              class="cursor-pointer hover:bg-surface-sunken"
              @click="$emit('pick', k)"
            >
              <td class="px-3 py-2 font-medium text-content-strong">{{ k.name }}</td>
              <td class="px-3 py-2 font-mono text-xs text-content-muted">{{ maskApiKey(k.key) }}</td>
              <td class="px-3 py-2">
                <GroupBadge
                  v-if="k.group"
                  :name="k.group.name"
