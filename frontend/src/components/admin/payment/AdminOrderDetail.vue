  ...
  >
    <div v-if="order" class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        <div>
          <p class="text-xs text-content-muted">{{ t('payment.orders.orderId') }}</p>
          <p class="font-mono text-sm font-medium text-content-strong">#{{ order.id }}</p>
        </div>
        <div>
          <p class="text-xs text-content-muted">{{ t('payment.orders.status') }}</p>
          <span :class="['badge', statusBadgeClass(order.status)]">
            {{ t('payment.status.' + order.status.toLowerCase(), order.status) }}
          </span>
        </div>
        <div>
          <p class="text-xs text-content-muted">{{ t('payment.orders.baseAmount') }}</p>
          <p class="text-sm font-medium text-content-strong">{{ paymentAmountSymbol }}{{ baseAmount.toFixed(2) }}</p>
        </div>
        <div v-if="order.fee_rate > 0">
          <p class="text-xs text-content-muted">{{ t('payment.orders.fee') }} ({{ order.fee_rate }}%)</p>
          <p class="text-sm font-medium text-content-strong">{{ paymentAmountSymbol }}{{ feeAmount.toFixed(2) }}</p>
        </div>
        <div>
          <p class="text-xs text-content-muted">{{ t('payment.orders.payAmount') }}</p>
          <p class="text-sm font-medium text-content-strong">{{ paymentAmountSymbol }}{{ order.pay_amount.toFixed(2) }}</p>
        </div>
        <div v-if="order.amount !== order.pay_amount">
          <p class="text-xs text-content-muted">{{ t('payment.orders.creditedAmount') }}</p>
          <p class="text-sm font-medium text-content-strong">{{ creditedAmountSymbol }}{{ order.amount.toFixed(2) }}</p>
        </div>
        <div>
          <p class="text-xs text-content-muted">{{ t('payment.orders.paymentMethod') }}</p>
          <p class="text-sm text-content">
            {{ t('payment.methods.' + order.payment_type, order.payment_type) }}
          </p>
        </div>
        <div>
          <p class="text-xs text-content-muted">{{ t('payment.admin.orderType') }}</p>
          <p class="text-sm text-content">
            {{ t('payment.admin.' + order.order_type + 'Order', order.order_type) }}
          </p>
        </div>
        <div>
          <p class="text-xs text-content-muted">{{ t('payment.orders.userId') }}</p>
          <p class="text-sm text-content">#{{ order.user_id }}</p>
        </div>
        <div>
          <p class="text-xs text-content-muted">{{ t('payment.orders.createdAt') }}</p>
          <p class="text-sm text-content">{{ formatDateTime(order.created_at) }}</p>
        </div>
        <div>
          <p class="text-xs text-content-muted">{{ t('payment.admin.expiresAt') }}</p>
          <p class="text-sm text-content">{{ formatDateTime(order.expires_at) }}</p>
        </div>
        <div v-if="order.paid_at">
          <p class="text-xs text-content-muted">{{ t('payment.admin.paidAt') }}</p>
          <p class="text-sm text-content">{{ formatDateTime(order.paid_at) }}</p>
        </div>
        <div v-if="order.completed_at">
          <p class="text-xs text-content-muted">{{ t('payment.admin.completedAt') }}</p>
          <p class="text-sm text-content">{{ formatDateTime(order.completed_at) }}</p>
        </div>
      </div>
