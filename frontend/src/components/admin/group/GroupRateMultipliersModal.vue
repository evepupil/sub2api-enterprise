  ...
  <BaseDialog :show="show" :title="t('admin.groups.rateMultipliersTitle')" width="wide" @close="handleClose">
    <div v-if="group" class="space-y-4">
      ...
      <div class="rounded-lg border border-gray-200 p-3 dark:border-dark-600">
        <!-- 添加用户 -->
        <h4 class="mb-2 text-sm font-medium text-content">
          ...
          <div class="relative flex-1">
            ...
            >
              ...
              >
                ...
                <span class="text-content-strong">{{ user.username || user.email }}</span>
        ...
        <div v-if="localEntries.length > 0" class="mt-3 flex items-center gap-3 border-t border-gray-100 pt-3 dark:border-dark-600">
          <span class="text-xs font-medium text-content-muted">{{ t('admin.groups.batchAdjust') }}</span>
      ...
      <div v-else>
        ...
        <div v-if="localEntries.length === 0" class="py-6 text-center text-sm text-content-subtle">
          {{ t('admin.groups.noRateMultipliers') }}
        </div>

        <div v-else>
          <!-- 表格 -->
          <div class="overflow-hidden rounded-lg border border-line-strong">
            <div class="max-h-[420px] overflow-auto">
              <table class="w-full min-w-max text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-gray-200 bg-gray-50 dark:border-dark-600 dark:bg-dark-700">
                    <th class="px-3 py-2 text-left text-xs font-medium text-content-muted">{{ t('admin.groups.columns.userEmail') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-content-muted">ID</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-content-muted">{{ t('admin.groups.columns.userName') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-content-muted">{{ t('admin.groups.columns.userNotes') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-content-muted">{{ t('admin.groups.columns.userStatus') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-content-muted">{{ t('admin.groups.columns.rateMultiplier') }}</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-dark-600">
                  ...
                  >
                    <td class="px-3 py-2 text-content-muted">{{ entry.user_email }}</td>
                    <td class="whitespace-nowrap px-3 py-2 text-content-subtle">{{ entry.user_id }}</td>
                    <td class="whitespace-nowrap px-3 py-2 text-content-strong">{{ entry.user_name || '-' }}</td>
                    <td class="max-w-[160px] truncate px-3 py-2 text-content-muted" :title="entry.user_notes">{{ entry.user_notes || '-' }}</td>
