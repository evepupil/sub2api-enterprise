    ...
    <div class="card flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      >
        <template #cell-model="{ row }">
          <span v-if="row.model" class="text-sm font-medium text-content-strong">{{ row.model }}</span>
          <span v-else class="text-sm text-content-subtle">-</span>
        </template>

        <template #cell-key_name="{ row }">
          <div class="text-sm">
            <span class="text-content-strong">{{ row.key_name || '-' }}</span>
            <span
        ...
        <template #cell-category="{ row }">
          <span class="text-sm text-content-strong">{{ t('usage.errors.categories.' + row.category) }}</span>
        </template>
        ...
        <template #cell-created_at="{ row }">
          <span class="text-sm text-content-muted">{{ formatDateTime(row.created_at) }}</span>
        </template>
