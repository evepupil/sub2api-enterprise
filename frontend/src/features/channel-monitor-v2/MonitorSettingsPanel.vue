<template>
  <section class="mx-auto w-full max-w-6xl space-y-5 px-1 py-2 sm:px-2">
    <header
      class="page-header mb-0 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white p-5 dark:bg-dark-800 dark:ring-dark-700 sm:p-6"
    >
      <div class="min-w-0">
        <h2 class="page-title flex items-center gap-2 text-xl font-black text-content-strong">
        ...
        <p class="page-description mt-1.5 text-xs text-content-muted">
    ...
    <div
      v-if="loading"
      class="card flex min-h-[200px] items-center justify-center !rounded-xl !border-0 text-sm text-gray-400 dark:ring-dark-700"
    >
      <span class="animate-pulse">{{ t('channelMonitorV2.settings.loading') }}</span>
    </div>

    <template v-else-if="draft">
      <div class="card divide-y divide-gray-100 !rounded-xl !border-0 dark:divide-dark-700 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="flex flex-wrap items-center justify-between gap-4 px-5 py-4">
          <div>
            <strong class="text-sm font-semibold text-content-strong">{{ t('channelMonitorV2.settings.enableTitle') }}</strong>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 px-5 py-4">
          <div>
            <strong class="text-sm font-semibold text-content-strong">{{ t('channelMonitorV2.settings.refreshTitle') }}</strong>
            <p class="mt-0.5 text-xs text-content-muted">{{ t('channelMonitorV2.settings.refreshHint') }}</p>
          </div>
      ...
      <div class="card overflow-hidden !rounded-xl !border-0 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="card-header !py-3">
          <h3 class="text-sm font-semibold text-content-strong">{{ t('channelMonitorV2.settings.platformsTitle') }}</h3>
          <p class="mt-0.5 text-xs text-content-muted">
        ...
        <div class="divide-y divide-line-subtle">
          ...
          >
            ...
            <strong class="text-sm font-medium text-content-strong">{{ platformLabel(platform.platform) }}</strong>
            <input
              class="input"
      ...
      <div class="card overflow-hidden !rounded-xl !border-0 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="card-header flex flex-wrap items-center justify-between gap-2 !py-3">
          <div>
            <h3 class="text-sm font-semibold text-content-strong">{{ t('channelMonitorV2.settings.groupsTitle') }}</h3>
            <p class="mt-0.5 text-xs text-content-muted">
              {{
        ...
        <div class="max-h-[min(40vh,280px)] overflow-y-auto px-3 py-2 sm:px-4">
          <div class="grid grid-cols-1 gap-1 sm:grid-cols-2">
            <label
              ...
              :key="group.id"
              class="flex cursor-pointer items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition hover:bg-surface-sunken/60"
            >
              <input
                type="checkbox"
              ...
              />
              <span class="min-w-0 flex-1 truncate font-medium text-content-strong">{{ group.name }}</span>
      ...
      <div class="card overflow-hidden !rounded-xl !border-0 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="card-header !py-3">
          <h3 class="text-sm font-semibold text-content-strong">{{ t('channelMonitorV2.settings.errorsTitle') }}</h3>
          <p class="mt-0.5 text-xs text-content-muted">
        ...
        <div class="max-h-[min(40vh,320px)] overflow-y-auto px-3 py-2 sm:px-4">
          <div class="grid grid-cols-1 gap-1 sm:grid-cols-2">
            ...
            >
              ...
              />
              <span class="min-w-0 flex-1 truncate font-medium text-content-strong">
      ...
      <div class="card overflow-hidden !rounded-xl !border-0 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="card-header !py-3">
          <h3 class="text-sm font-semibold text-content-strong">{{ t('channelMonitorV2.settings.healthTitle') }}</h3>
          <p class="mt-0.5 text-xs text-content-muted">
      ...
      <div class="space-y-2">
        ...
        <div class="rounded-2xl border border-gray-200 bg-gray-50/80 px-4 py-3 text-xs text-gray-600 dark:border-dark-600 dark:bg-dark-800/50 dark:text-gray-300">
          <p class="font-medium text-content-strong">{{ t('channelMonitorV2.settings.userContractTitle') }}</p>
