                ...
                <Teleport to="body">
                  ...
                  >
                    <div class="overflow-y-auto p-2" :style="{ maxHeight: `${accountToolsDropdownPosition.maxHeight}px` }">
                      ...
                      <div class="my-2 border-t border-line-subtle"></div>
                      <div class="px-2 py-2">
                        <div class="text-xs font-semibold uppercase tracking-wide text-content-subtle">
        ...
        >
          ...
          <template #cell-name="{ row, value }">
            <div class="flex flex-col">
              ...
              <span v-else class="font-medium text-content-strong">{{ value }}</span>
              <span
                v-if="accountDisplayEmail(row)"
                class="text-xs text-content-muted truncate max-w-[200px]"
          ...
          <template #cell-notes="{ value }">
            <span v-if="value" :title="value" class="block max-w-xs truncate text-sm text-content-muted">{{ value }}</span>
          ...
          <template #cell-proxy="{ row }">
            <div class="flex flex-col gap-1">
              <div v-if="row.proxy" class="flex items-center gap-2">
                <span class="text-sm text-content">{{ row.proxy.name }}</span>
                <span v-if="row.proxy.country_code" class="text-xs text-content-muted">
                  ({{ row.proxy.country_code }})
                </span>
              </div>
              <span v-else class="text-sm text-content-subtle">-</span>
              <div v-if="row.proxy && row.proxy.expires_at" class="flex items-center gap-2 text-xs">
                <span class="text-content-muted">{{ formatDateTime(row.proxy.expires_at) }}</span>
              ...
              <div v-if="row.proxy_fallback_origin_id" class="flex items-center gap-1">
                ...
                <button class="text-xs px-1.5 py-0.5 rounded border border-line-strong text-content-muted hover:bg-surface-sunken" @click="onRevertFallback(row)">{{ t('admin.accounts.revertProxy') }}</button>
              </div>
            </div>
          </template>
          <template #cell-rate_multiplier="{ row }">
            <span class="inline-flex items-center gap-1 text-sm font-mono text-content">
          ...
          <template #cell-scheduler_score="{ row }">
            <div v-if="getSchedulerScoreRows(row).length" class="flex min-w-[7rem] flex-col gap-0.5 font-mono text-[11px] leading-4">
              <div
                ...
                class="flex items-center gap-1 whitespace-nowrap text-content"
                :title="`${formatSchedulerScoreGroup(score)} / ${formatSchedulerScore(score.base_score)} / ${formatStickySchedulerScore(score)}`"
              >
                <span class="max-w-[4.75rem] truncate text-content-muted">{{ formatSchedulerScoreGroup(score) }}</span>
          ...
          <template #cell-last_used_at="{ value }">
            <span class="text-sm text-content-muted">{{ formatRelativeTime(value) }}</span>
          </template>
          ...
          <template #cell-expires_at="{ row, value }">
            <div class="flex flex-col items-start gap-1">
              <span class="text-sm text-content-muted">{{ formatExpiresAt(value) }}</span>
