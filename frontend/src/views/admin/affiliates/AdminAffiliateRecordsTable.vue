      ...
      <template #table>
        ...
        >
          ...
          <template #cell-aff_code="{ row }">
            <span class="font-mono text-sm text-content">{{ row.aff_code || '-' }}</span>
          </template>
          <template #cell-order="{ row }">
            <div class="space-y-0.5">
              <div class="font-mono text-sm text-content-strong">#{{ row.order_id }}</div>
              <div class="max-w-56 truncate text-sm text-content-muted">{{ row.out_trade_no }}</div>
            </div>
          </template>
          ...
          <template #cell-pay_amount="{ row }">
            <span class="text-sm text-content-strong">¥{{ formatAmount(row.pay_amount) }}</span>
          </template>
          ...
          <template #cell-created_at="{ row }">
            <span class="text-sm text-content">{{ formatDateTime(row.created_at) }}</span>
          </template>
        </DataTable>
      </template>
    ...
    >
      ...
      <div v-else-if="selectedOverview" class="space-y-4">
        <div class="rounded-lg border border-gray-100 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800">
          <div class="font-mono text-sm text-content-strong">#{{ selectedOverview.user_id }}</div>
          <div class="mt-1 text-sm font-medium text-content-strong">{{ selectedOverview.email || '-' }}</div>
          <div class="mt-0.5 text-sm text-content-muted">{{ selectedOverview.username || '-' }}</div>
        </div>
...
const UserCell = defineComponent({
  ...
  setup(cellProps, { emit }) {
    return () => h('div', { class: 'space-y-0.5' }, [
      h('div', { class: 'font-mono text-sm text-content-strong' }, `#${cellProps.id}`),
      h(cellProps.clickable ? 'button' : 'div', {
        class: cellProps.clickable
          ...
          : 'max-w-56 truncate text-sm text-content',
        type: cellProps.clickable ? 'button' : undefined,
        onClick: cellProps.clickable ? () => emit('open', cellProps.id) : undefined,
      }, cellProps.email || '-'),
      h('div', { class: 'max-w-56 truncate text-sm text-content-muted' }, cellProps.username || '-'),
    ])
  },
})
  ...
  setup(amountProps) {
    return () => {
      ...
      if (value === null || value === undefined) {
        return h('span', { class: 'text-sm text-content-subtle' }, '-')
      }
...
const OverviewStat = defineComponent({
  ...
  setup(statProps) {
    return () => h('div', { class: 'rounded-lg border border-gray-100 bg-white p-3 dark:border-dark-700 dark:bg-dark-900' }, [
      h('div', { class: 'text-sm text-content-muted' }, statProps.label),
      h('div', {
        class: statProps.mono
          ? 'mt-1 font-mono text-base font-semibold text-content-strong'
          : 'mt-1 text-base font-semibold text-content-strong',
      }, statProps.value),
    ])
  },
})
