...
<template>
  <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-dark-800 dark:ring-dark-700">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
      <div>
        <h3 class="text-sm font-bold text-content-strong">{{ t('admin.ops.alertEvents.title') }}</h3>
        <p class="mt-1 text-xs text-content-muted">{{ t('admin.ops.alertEvents.description') }}</p>
      </div>
    ...
    <div v-if="loading" class="flex items-center gap-2 text-sm text-content-muted">
    ...
    <div v-else class="overflow-hidden rounded-xl border border-line-subtle">
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <div v-if="!isDesktopViewport" class="divide-y divide-line-subtle">
          <div
            v-for="row in events"
            :key="row.id"
            class="cursor-pointer space-y-2 p-4 hover:bg-surface-sunken/50"
          ...
          >
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span class="ml-auto text-[11px] text-content-muted">
                {{ formatDateTime(row.fired_at || row.created_at) }}
              </span>
            </div>
            <div class="text-xs font-semibold text-content-strong">{{ row.title || '-' }}</div>
            <div v-if="row.description" class="line-clamp-2 text-[11px] text-content-muted">
              {{ row.description }}
            </div>
            <div class="flex flex-wrap items-center justify-between gap-2 text-[11px] text-content-muted">
            ...
            <div class="text-[11px] text-content-subtle">{{ formatDimensionsSummary(row) }}</div>
          </div>
        </div>
        <table v-else class="min-w-full divide-y divide-line-subtle">
          <thead class="sticky top-0 z-10 bg-surface-sunken">
            <tr>
              <th class="px-3 py-2 text-left text-[11px] font-bold text-content-muted">
              ...
              <th class="px-3 py-2 text-right text-[11px] font-bold text-content-muted">
          ...
          <tbody class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-800">
            <tr
              v-for="row in events"
              :key="row.id"
              class="cursor-pointer hover:bg-surface-sunken/50"
              @click="openDetail(row)"
              :title="row.title || ''"
            >
              ...
              <td class="min-w-[260px] px-3 py-2 text-xs text-content">
                <div class="font-semibold truncate max-w-[360px]">{{ row.title || '-' }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-content-muted">
                  {{ row.description }}
                </div>
              </td>
              <td class="whitespace-nowrap px-3 py-2 text-xs text-content-muted">
              ...
              <td class="whitespace-nowrap px-3 py-2 text-[11px] text-content-muted">
              ...
              <td class="whitespace-nowrap px-3 py-2 text-right text-xs">
                ...
                >
                  ...
                  <Icon
                    ...
                    class="text-content-subtle"
                  />
                  <span class="text-[11px] font-bold text-content-muted">
                    {{ row.email_sent ? t('admin.ops.alertEvents.table.emailSent') : t('admin.ops.alertEvents.table.emailIgnored') }}
                  </span>
                </span>
              </td>
        ...
        <div v-if="loadingMore" class="flex items-center justify-center gap-2 py-3 text-xs text-content-muted">
          <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
    ...
    <BaseDialog
      ...
      @close="closeDetail"
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-10 text-sm text-content-muted">
        {{ t('admin.ops.alertEvents.detail.loading') }}
      </div>

      <div v-else-if="!selected" class="py-10 text-center text-sm text-content-muted">
      ...
      <div v-else class="space-y-5">
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div class="mt-2 text-sm font-semibold text-content-strong">
                {{ selected.title || '-' }}
              </div>
              <div v-if="selected.description" class="mt-1 whitespace-pre-wrap text-xs text-content-muted">
            ...
            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg bg-white px-2 py-1 ring-1 ring-gray-200 dark:bg-dark-800 dark:ring-dark-700">
                <span class="text-[11px] font-bold text-content-muted">{{ t('admin.ops.alertEvents.detail.silence') }}</span>
                <Select
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
              ...
              <div class="mt-1 text-sm font-medium text-content-strong">{{ formatDateTime(selected.fired_at || selected.created_at) }}</div>
            </div>
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.alertEvents.detail.resolvedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-content-strong">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' }}</div>
            </div>
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.alertEvents.detail.ruleId') }}</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-bold text-content-strong">#{{ selected.rule_id }}</div>
                <a
                  class="inline-flex items-center gap-1 rounded-md bg-white px-2 py-1 text-[11px] font-bold text-gray-700 ring-1 ring-gray-200 hover:bg-surface-sunken dark:text-gray-200 dark:ring-dark-700 dark:hover:bg-dark-700"
            ...
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
              ...
              <div class="mt-1 text-sm text-content-strong">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-800">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="text-sm font-bold text-content-strong">{{ t('admin.ops.alertEvents.detail.historyTitle') }}</div>
              <div class="mt-0.5 text-xs text-content-muted">{{ t('admin.ops.alertEvents.detail.historyHint') }}</div>
            </div>
            <Select :model-value="historyRange" :options="historyRangeOptions" class="w-[140px]" @change="historyRange = String($event || '7d')" />
          </div>

          <div v-if="historyLoading" class="py-6 text-center text-xs text-content-muted">
            {{ t('admin.ops.alertEvents.detail.historyLoading') }}
          </div>
          <div v-else-if="history.length === 0" class="py-6 text-center text-xs text-content-muted">
            {{ t('admin.ops.alertEvents.detail.historyEmpty') }}
          </div>
          <div v-else class="overflow-hidden rounded-lg border border-line-subtle">
            <table class="min-w-full divide-y divide-line-subtle">
              <thead class="bg-surface-sunken">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-bold text-content-muted">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold text-content-muted">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold text-content-muted">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-line-subtle">
                <tr v-for="it in history" :key="it.id" class="hover:bg-surface-sunken/50">
                  <td class="px-3 py-2 text-xs text-content-muted">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
