      ...
      <div class="flex h-full min-h-0 flex-col">
        ...
        <div v-if="loading" class="flex flex-1 items-center justify-center py-16">
          <div class="flex flex-col items-center gap-3">
            ...
            <span class="text-sm font-medium text-content-muted">{{ t('common.loading') }}</span>
          </div>
        </div>
        ...
        <div v-else class="flex min-h-0 flex-1 flex-col">
          <div v-if="items.length === 0" class="rounded-xl border border-dashed border-gray-200 p-10 text-center dark:border-dark-700">
            <div class="text-sm font-medium text-content-muted">{{ t('admin.ops.requestDetails.empty') }}</div>
          ...
          <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-line-subtle">
            <div class="min-h-0 flex-1 overflow-auto">
              <div v-if="!isDesktopViewport" class="divide-y divide-line-subtle">
                <div v-for="(row, idx) in items" :key="idx" class="space-y-2 p-4">
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span class="text-xs font-medium text-content">{{ (row.platform || 'unknown').toUpperCase() }}</span>
                    <span class="ml-auto text-[11px] text-content-muted">{{ formatDateTime(row.created_at) }}</span>
                  </div>
                  <div class="break-all text-xs text-content-muted">{{ row.model || '-' }}</div>
                  ...
                  <div v-if="row.request_id" class="flex items-center gap-2">
                    <span class="min-w-0 flex-1 truncate font-mono text-[11px] text-content" :title="row.request_id">
                      {{ row.request_id }}
                    </span>
                    <button
              ...
              <tbody class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-800">
                <tr v-for="(row, idx) in items" :key="idx" class="hover:bg-surface-sunken/50">
                  ...
                  <td class="max-w-[240px] truncate px-3 py-2 text-xs text-content-muted" :title="row.model || ''">
                    {{ row.model || '-' }}
                  </td>
                  <td class="whitespace-nowrap px-3 py-2 text-xs text-content-muted">
                  ...
                  <td class="px-3 py-2">
                    <div v-if="row.request_id" class="flex items-center gap-2">
                      <span class="max-w-[220px] truncate font-mono text-[11px] text-content" :title="row.request_id">
                        {{ row.request_id }}
                      </span>
                      <button
