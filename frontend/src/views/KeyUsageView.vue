  ...
  <div class="relative flex min-h-screen flex-col bg-surface">
    ...
    <header class="relative z-20 px-6 py-4">
      <nav class="mx-auto flex max-w-6xl items-center justify-between">
        <router-link to="/home" class="flex items-center gap-3">
          ...
          <span class="text-lg font-semibold tracking-tight text-content-strong">{{ siteName }}</span>
        </router-link>
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-6 py-12">
      ...
      <div class="text-center mb-12">
        <h1 class="text-3xl sm:text-4xl font-bold tracking-tight mb-3 text-content-strong">
          {{ t('keyUsage.title') }}
        </h1>
        <p class="text-content-muted text-base max-w-md mx-auto">
          {{ t('keyUsage.subtitle') }}
        </p>
      </div>
      ...
      <div class="max-w-xl mx-auto mb-14">
        <div class="flex gap-3">
          <div class="flex-1 relative">
            <div class="absolute left-4 top-1/2 -translate-y-1/2 text-content-subtle">
            ...
            <input
              ...
              class="input-ring h-[38px] w-full rounded-lg border border-line-strong bg-surface-raised pl-11 pr-11 text-sm text-content-strong transition-colors placeholder:text-content-subtle"
              @keydown.enter="queryKey"
            />
            <button
              @click="keyVisible = !keyVisible"
              class="absolute right-3 top-1/2 -translate-y-1/2 text-content-subtle transition-colors hover:text-content-strong"
            >
        ...
        <p class="text-xs text-content-subtle mt-3 text-center">
        ...
        <div v-if="showDatePicker" class="mt-4">
          <div class="flex flex-wrap items-center gap-2 justify-center">
            <span class="text-xs text-content-muted">{{ t('keyUsage.dateRange') }}</span>
            <button
              ...
              :class="currentRange === range.key
                ...
                : 'border-line-strong bg-surface-raised text-content hover:border-line-strong hover:bg-surface-sunken'"
            >{{ range.label }}</button>
            <div v-if="currentRange === 'custom'" class="flex items-center gap-2 ml-1">
              <input
                v-model="customStartDate"
                type="date"
                class="input-ring h-[30px] rounded-lg border border-line-strong bg-surface-raised px-2 text-xs text-content-strong"
              />
      ...
      <div v-if="showResults">
        ...
        <div v-if="showLoading" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div class="rounded-xl border border-line-subtle bg-surface-raised p-5">
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          <div v-if="statusInfo" class="fade-up flex items-center justify-center mb-2">
            <div class="inline-flex items-center gap-2 rounded-full border border-line-subtle bg-surface-raised px-4 py-1.5">
              <span
                class="w-2.5 h-2.5 rounded-full pulse-dot"
                :class="statusInfo.isActive ? 'bg-emerald-500' : 'bg-rose-500'"
              ></span>
              <span class="text-sm font-medium text-content-strong">{{ statusInfo.label }}</span>
              <span class="text-xs text-content-subtle">|</span>
              <span class="text-xs text-content-muted">{{ statusInfo.statusText }}</span>
            </div>
          </div>
          ...
          <div v-if="ringItems.length > 0" :class="ringGridClass">
            <div
              ...
              :key="i"
              class="fade-up rounded-xl border border-line-subtle bg-surface-raised p-5 transition-colors duration-150 hover:border-line-strong"
              :class="`fade-up-delay-${Math.min(i + 1, 4)}`"
            >
              <div class="flex items-center justify-between mb-6">
                <h3 class="text-sm font-semibold uppercase tracking-wider text-content-muted">
                  {{ ring.title }}
                </h3>
                <!-- Clock icon -->
                <svg v-if="ring.iconType === 'clock'" class="w-5 h-5 text-content-subtle" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                </svg>
                <!-- Calendar icon -->
                <svg v-else-if="ring.iconType === 'calendar'" class="w-5 h-5 text-content-subtle" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
                </svg>
                <!-- Dollar icon -->
                <svg v-else class="w-5 h-5 text-content-subtle" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div class="flex justify-center">
                <div class="relative">
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <template v-else>
                      <span class="text-3xl font-bold tabular-nums text-content-strong">
                        {{ displayPcts[i] ?? 0 }}%
                      </span>
                      <span class="text-xs text-content-muted mt-0.5">{{ t('keyUsage.used') }}</span>
                      <span
                        class="text-sm font-semibold mt-1 tabular-nums"
                        :style="{ color: RING_GRADIENTS[i % 4].from }"
                      >{{ ring.amount }}</span>
                      <p v-if="ring.resetAt && formatResetTime(ring.resetAt)" class="text-xs text-content-subtle mt-0.5 tabular-nums">
          ...
          <div
            ...
            class="fade-up fade-up-delay-3 overflow-hidden rounded-xl border border-line-subtle bg-surface-raised"
          >
            <div class="border-b border-line-subtle px-5 py-3.5">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-content-muted">{{ t('keyUsage.detailInfo') }}</h3>
            </div>
            <div class="divide-y divide-line-subtle">
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-sm text-content">{{ row.label }}</span>
                </div>
                <span class="text-sm font-semibold tabular-nums" :class="row.valueClass || 'text-content-strong'">
                  {{ row.value }}
                </span>
              </div>
            </div>
          ...
          >
            <div class="border-b border-line-subtle px-5 py-3.5">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-content-muted">{{ t('keyUsage.tokenStats') }}</h3>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-px bg-surface-sunken">
              <div
                v-for="(cell, i) in usageStatCells"
                :key="i"
                class="bg-surface-raised px-5 py-3"
              >
                <div class="text-xs text-content-muted mb-1">{{ cell.label }}</div>
                <div class="text-sm font-semibold tabular-nums text-content-strong">{{ cell.value }}</div>
              </div>
            </div>
          </div>
          ...
          <div
            v-if="showDailyUsage"
            class="fade-up fade-up-delay-4 overflow-hidden rounded-xl border border-line-subtle bg-surface-raised"
          >
            <div class="flex flex-col gap-3 border-b border-line-subtle px-5 py-3.5 sm:flex-row sm:items-center sm:justify-between">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-content-muted">{{ t('keyUsage.dailyDetail') }}</h3>
              <div class="segmented">
                <button
            ...
            <div v-if="dailyUsageRows.length > 0" class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-line-subtle bg-surface-sunken">
                    <th class="h-[34px] px-3 text-left text-xs font-medium text-content-muted">{{ t('keyUsage.date') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.requests') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.cacheWriteTokens') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                  ...
                  >
                    <td class="h-9 px-3 py-0 text-sm font-medium whitespace-nowrap text-content-strong">{{ row.date }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right text-content">{{ fmtNum(row.requests) }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right text-content">{{ fmtNum(row.input_tokens) }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right text-content">{{ fmtNum(row.output_tokens) }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right text-content">{{ fmtNum(row.cache_read_tokens) }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right text-content">{{ fmtNum(row.cache_write_tokens) }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right font-medium text-content-strong">{{ usd(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div v-else class="px-5 py-10 text-center text-sm text-content-muted">
          ...
          >
            <div class="border-b border-line-subtle px-5 py-3.5">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-content-muted">{{ t('keyUsage.modelStats') }}</h3>
            </div>
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-line-subtle bg-surface-sunken">
                    <th class="h-[34px] px-3 text-left text-xs font-medium text-content-muted">{{ t('keyUsage.model') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.requests') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.cacheCreationTokens') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.totalTokens') }}</th>
                    <th class="h-[34px] px-3 text-right text-xs font-medium text-content-muted">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    class="border-b border-line-subtle last:border-b-0"
                  >
                    <td class="h-9 px-3 py-0 text-sm font-medium whitespace-nowrap text-content-strong">{{ m.model || '-' }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right text-content">{{ fmtNum(m.requests) }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right text-content">{{ fmtNum(m.input_tokens) }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right text-content">{{ fmtNum(m.output_tokens) }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right text-content">{{ fmtNum(m.cache_creation_tokens) }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right text-content">{{ fmtNum(m.cache_read_tokens) }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right text-content">{{ fmtNum(m.total_tokens) }}</td>
                    <td class="h-9 px-3 py-0 text-sm tabular-nums text-right font-medium text-content-strong">{{ usd(m.actual_cost != null ? m.actual_cost : m.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    <footer class="relative z-10 border-t border-line-subtle px-5 py-4">
      <div class="mx-auto flex max-w-6xl flex-col items-center justify-center gap-4 text-center sm:flex-row sm:text-left">
        ...
        <div class="flex items-center gap-4">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            class="text-sm text-content-muted transition-colors hover:text-content-strong"
          >{{ t('home.docs') }}</a>
          <a
            :href="githubUrl"
            target="_blank"
...
const ringGridClass = computed(() => {
  ...
  if (len === 1) return 'grid grid-cols-1 max-w-md mx-auto gap-3'
  if (len === 2) return 'grid grid-cols-1 md:grid-cols-2 gap-3'
...
.input-ring:focus {
  box-shadow: 0 0 0 3px rgba(27, 75, 168, 0.2);
  border-color: #1b4ba8;
  outline: none;
}
