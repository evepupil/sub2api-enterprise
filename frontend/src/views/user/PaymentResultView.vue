    ...
    <div class="w-full max-w-md space-y-6">
      ...
      <template v-else>
        ...
        <div class="text-center">
          ...
          <h2 class="mt-4 text-2xl font-bold text-content-strong">
            {{ statusTitle }}
          </h2>
          <p v-if="isPending" class="mt-2 text-sm text-content-muted">
        ...
        <div v-if="order" class="rounded-xl bg-white p-5 shadow-sm dark:bg-dark-800">
          <div class="space-y-3 text-sm">
            <div v-if="hasOrderId(order)" class="flex justify-between">
              ...
              <span class="font-medium text-content-strong">#{{ order.id }}</span>
            </div>
            <div v-if="order.out_trade_no" class="flex justify-between">
              <span class="text-content-muted">{{ t('payment.orders.orderNo') }}</span>
              <span class="font-medium text-content-strong">{{ order.out_trade_no }}</span>
            </div>
            <div v-if="hasAmountFields(order)" class="flex justify-between">
              <span class="text-content-muted">{{ t('payment.orders.baseAmount') }}</span>
              <span class="font-medium text-content-strong">{{ formatGatewayAmount(baseAmount) }}</span>
            </div>
            <div v-if="hasAmountFields(order) && order.fee_rate > 0" class="flex justify-between">
              <span class="text-content-muted">{{ t('payment.orders.fee') }} ({{ order.fee_rate }}%)</span>
              <span class="font-medium text-content-strong">{{ formatGatewayAmount(feeAmount) }}</span>
            </div>
            <div v-if="hasAmountFields(order)" class="flex justify-between">
            ...
            <div v-if="hasAmountFields(order) && order.amount !== order.pay_amount" class="flex justify-between">
              <span class="text-content-muted">{{ t('payment.orders.creditedAmount') }}</span>
              <span class="font-medium text-content-strong">{{ order.order_type === 'balance' ? '$' + order.amount.toFixed(2) : formatGatewayAmount(order.amount) }}</span>
            </div>
            <div v-if="hasPaymentType(order)" class="flex justify-between">
              <span class="text-content-muted">{{ t('payment.orders.paymentMethod') }}</span>
              <span class="font-medium text-content-strong">{{ t(paymentMethodI18nKey(order.payment_type), normalizedOrderPaymentType(order.payment_type)) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-content-muted">{{ t('payment.orders.status') }}</span>
        ...
        <div v-else-if="returnInfo" class="rounded-xl bg-white p-5 shadow-sm dark:bg-dark-800">
          <div class="space-y-3 text-sm">
            <div v-if="returnInfo.outTradeNo" class="flex justify-between">
              <span class="text-content-muted">{{ t('payment.orders.orderId') }}</span>
              <span class="font-medium text-content-strong">{{ returnInfo.outTradeNo }}</span>
            </div>
            <div v-if="returnInfo.money" class="flex justify-between">
              <span class="text-content-muted">{{ t('payment.orders.payAmount') }}</span>
              <span class="font-medium text-content-strong">{{ formatGatewayAmount(Number(returnInfo.money) || 0) }}</span>
            </div>
            <div v-if="returnInfo.type" class="flex justify-between">
              <span class="text-content-muted">{{ t('payment.orders.paymentMethod') }}</span>
              <span class="font-medium text-content-strong">{{ t(paymentMethodI18nKey(returnInfo.type), normalizedOrderPaymentType(returnInfo.type)) }}</span>
            </div>
          </div>
        </div>
        <!-- Actions -->
