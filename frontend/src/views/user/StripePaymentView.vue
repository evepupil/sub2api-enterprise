<template>
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-surface-sunken' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        ...
        <h3 class="text-lg font-semibold text-content-strong">{{ t('payment.stripeLoadFailed') }}</h3>
        <p class="mt-2 text-sm text-content-muted">{{ initError }}</p>
      ...
      <template v-else>
        ...
        <div v-if="order" class="card overflow-hidden">
          <div class="bg-gradient-to-br from-[#635bff] to-[#4f46e5] px-5 py-4 text-center">
        ...
        <template v-if="wechatQrUrl">
          <div class="card p-5">
            <div class="flex flex-col items-center space-y-4">
              <p class="text-lg font-semibold text-content-strong">{{ t('payment.qr.scanWxpay') }}</p>
              ...
              <p class="text-center text-sm text-content-muted">{{ t('payment.qr.scanWxpayHint') }}</p>
            </div>
          </div>
          <div class="card p-4 text-center">
            <p class="text-sm text-content-muted">{{ t('payment.qr.waitingPayment') }}</p>
          </div>
        </template>
        ...
        <template v-else-if="stripeSuccess">
          <div class="card p-6 text-center">
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <p class="text-sm text-content-muted">{{ t('payment.stripeSuccessProcessing') }}</p>
            </div>
          </div>
        </template>
