      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <template v-if="activeTab === 'recharge'">
            ...
            <div class="card p-5">
              <p class="text-xs font-medium text-content-subtle">{{ t('payment.rechargeAccount') }}</p>
              <p class="mt-1 text-base font-semibold text-content-strong">{{ user?.username || '' }}</p>
              <p class="mt-0.5 text-sm font-medium text-green-600 dark:text-green-400">{{ t('payment.currentBalance') }}: {{ user?.balance?.toFixed(2) || '0.00' }}</p>
            </div>
            <div v-if="enabledMethods.length === 0" class="card py-16 text-center">
              <p class="text-content-muted">{{ t('payment.notAvailable') }}</p>
            </div>
            <template v-else>
            <div class="card p-5">
              <AmountInput
            ...
            <div v-if="enabledMethods.length >= 1" class="card p-5">
            ...
            <div v-if="validAmount > 0" class="card p-5">
              <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                  <span class="text-content-muted">{{ t('payment.paymentAmount') }}</span>
                  <span class="text-content-strong">{{ formatSelectedPaymentAmount(validAmount) }}</span>
                </div>
                <div v-if="feeRate > 0" class="flex justify-between">
                  <span class="text-content-muted">{{ t('payment.fee') }} ({{ feeRate }}%)</span>
                  <span class="text-content-strong">{{ formatSelectedPaymentAmount(feeAmount) }}</span>
                </div>
                <div v-if="feeRate > 0" class="flex justify-between border-t border-gray-200 pt-2 dark:border-dark-600">
                  <span class="font-medium text-content">{{ t('payment.actualPay') }}</span>
                  <span class="text-lg font-bold text-primary-600 dark:text-primary-400">{{ formatSelectedPaymentAmount(totalAmount) }}</span>
                </div>
                <div v-if="balanceRechargeMultiplier !== 1" class="flex justify-between" :class="{ 'border-t border-gray-200 pt-2 dark:border-dark-600': feeRate <= 0 }">
                  <span class="text-content-muted">{{ t('payment.creditedBalance') }}</span>
                  <span class="text-content-strong">${{ creditedAmount.toFixed(2) }}</span>
                </div>
          ...
          <template v-else-if="activeTab === 'subscription'">
            ...
            <template v-if="selectedPlan">
              <div class="card p-5">
                ...
                <div class="mb-3 flex flex-wrap items-center gap-2">
                  ...
                  <h3 class="text-lg font-bold text-content-strong">{{ selectedPlan.name }}</h3>
                </div>
                <!-- Price -->
                <div class="flex items-baseline gap-2">
                  ...
                  <span class="text-sm text-content-muted">/ {{ planValiditySuffix }}</span>
                </div>
                <!-- Description -->
                <p v-if="selectedPlan.description" class="mt-2 text-sm leading-relaxed text-content-muted">
                  {{ selectedPlan.description }}
                </p>
                <!-- Rate + Limits grid -->
                <div class="mt-3 grid grid-cols-2 gap-3">
                  <div>
                    <span class="text-xs text-content-subtle">{{ t('payment.planCard.rate') }}</span>
                    <div class="flex items-baseline">
                      <span :class="['text-lg font-bold', planTextClass]">×{{ selectedPlan.rate_multiplier ?? 1 }}</span>
                    </div>
                  </div>
                  <div v-if="planHasPeakRate(selectedPlan)">
                    <span class="text-xs text-content-subtle">{{ t('payment.planCard.peakRate') }}</span>
                    <div class="text-sm font-semibold text-amber-700 dark:text-amber-300">
                      {{ planPeakRateLabel(selectedPlan) }}
                    </div>
                  </div>
                  <div v-if="selectedPlan.daily_limit_usd != null">
                    <span class="text-xs text-content-subtle">{{ t('payment.planCard.dailyLimit') }}</span>
                    <div class="text-lg font-semibold text-content">${{ selectedPlan.daily_limit_usd }}</div>
                  </div>
                  <div v-if="selectedPlan.weekly_limit_usd != null">
                    <span class="text-xs text-content-subtle">{{ t('payment.planCard.weeklyLimit') }}</span>
                    <div class="text-lg font-semibold text-content">${{ selectedPlan.weekly_limit_usd }}</div>
                  </div>
                  <div v-if="selectedPlan.monthly_limit_usd != null">
                    <span class="text-xs text-content-subtle">{{ t('payment.planCard.monthlyLimit') }}</span>
                    <div class="text-lg font-semibold text-content">${{ selectedPlan.monthly_limit_usd }}</div>
                  </div>
                  <div v-if="selectedPlan.daily_limit_usd == null && selectedPlan.weekly_limit_usd == null && selectedPlan.monthly_limit_usd == null">
                    <span class="text-xs text-content-subtle">{{ t('payment.planCard.quota') }}</span>
              ...
              <div v-if="feeRate > 0 && selectedPlan.price > 0" class="card p-5">
                <div class="space-y-2 text-sm">
                  <div class="flex justify-between">
                    <span class="text-content-muted">{{ t('payment.amountLabel') }}</span>
                    <span class="text-content-strong">{{ formatSelectedPaymentAmount(subPaymentAmount) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-content-muted">{{ t('payment.fee') }} ({{ feeRate }}%)</span>
                    <span class="text-content-strong">{{ formatSelectedPaymentAmount(subFeeAmount) }}</span>
                  </div>
            ...
            <template v-else>
              <div v-if="checkout.plans.length === 0" class="card py-16 text-center">
                ...
                <p class="text-content-muted">{{ t('payment.noPlans') }}</p>
              </div>
              ...
              <div v-if="activeSubscriptions.length > 0">
                <p class="mb-2 text-xs font-medium text-content-subtle">{{ t('payment.activeSubscription') }}</p>
                <div class="space-y-2">
                  <div v-for="sub in activeSubscriptions" :key="sub.id"
                    ...
                    <div class="min-w-0 flex-1">
                      <div class="flex items-center gap-1.5">
                        <span class="truncate text-xs font-semibold text-content-strong">{{ sub.group?.name || t('payment.groupFallback', { id: sub.group_id }) }}</span>
                        <span :class="['shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-medium', platformBadgeLightClass(sub.group?.platform || '')]">{{ platformLabel(sub.group?.platform || '') }}</span>
                      </div>
                      <div class="flex flex-wrap gap-x-3 text-[11px] text-content-subtle">
      ...
      <Transition name="modal">
        <div v-if="showRenewalModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" @click.self="closeRenewalModal">
          <div class="relative w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-dark-700 dark:bg-dark-900">
            ...
            <h3 class="mb-4 text-lg font-semibold text-content-strong">{{ t('payment.selectPlan') }}</h3>
            <div class="space-y-4">
