  ...
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div class="flex flex-col gap-3">
          ...
          >
            ...
            >
              <template #count>
                <span class="font-medium text-content-strong">{{ selectedJobIds.size }}</span>
              </template>
            </i18n-t>
        ...
        >
	            ...
	            <div class="flex w-[220px] items-start gap-1" :class="row.is_child ? 'pl-6' : ''">
	              ...
	              <button type="button" class="min-w-0 flex-1 rounded-lg py-1 text-left transition-colors hover:bg-gray-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/30 dark:hover:bg-dark-700" @click="selectJob(row.id)">
	                <span
	                  ...
	                  :class="row.task_name ? 'text-content-strong' : 'text-content-muted'"
                >
                  ...
                  <span v-if="row.is_child" class="flex-shrink-0 rounded-full bg-amber-50 px-2 py-0.5 text-xs font-normal text-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
	                ...
	                <span class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-content-muted">
          ...
          <template #cell-model="{ row }">
	            <div class="mx-auto max-w-[180px] text-center">
	              <p class="truncate text-sm text-content" :title="row.model">{{ row.model }}</p>
	            </div>
	          </template>

          <template #cell-api_key_name="{ value }">
            <span class="block truncate text-center text-sm text-content">
          ...
          <template #cell-counts="{ row }">
            <div class="flex items-center justify-center gap-2 text-sm tabular-nums">
              ...
              <span :class="displayJob(row).fail_count > 0 ? 'text-red-600 dark:text-red-300' : 'text-content-subtle'">{{ displayJob(row).fail_count }}</span>
              <span class="text-xs text-content-subtle">{{ t('batchImage.list.totalCount', { n: displayJob(row).item_count }) }}</span>
            </div>
          </template>

          <template #cell-cost="{ row }">
            <span class="block text-center text-sm text-content">
              {{ costLabel(displayJob(row)) }}
            </span>
          </template>

          <template #cell-downloaded="{ row }">
            <span class="block text-center text-sm" :class="row.downloaded_at ? 'text-emerald-700 dark:text-emerald-300' : 'text-content-muted'">
          ...
          <template #empty>
            <div class="flex min-h-[260px] flex-col items-center justify-center py-6 md:min-h-[300px]">
              <Icon name="sparkles" size="xl" class="mb-4 h-12 w-12 text-content-subtle" />
              <p class="text-lg font-medium text-content-strong">{{ t('batchImage.list.empty') }}</p>
      ...
      <template #pagination>
        ...
        >
          <div class="flex flex-wrap items-center gap-3 text-sm text-content">
    ...
    <Teleport to="body">
      ...
      >
        <div class="mb-2 flex items-center justify-between gap-3">
          <span class="text-xs font-medium text-content-muted">{{ t('batchImage.promptPopover.title') }}</span>
          <button
            type="button"
    ...
    <BaseDialog :show="!!currentJob" :title="t('batchImage.detail.title')" width="extra-wide" @close="closeDetail">
      <div v-if="currentJob" class="space-y-4">
        <div class="rounded-lg border border-gray-200 bg-gray-50/70 px-4 py-3 dark:border-dark-700 dark:bg-dark-900/40">
          <div class="grid gap-x-6 gap-y-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="min-w-0 text-center">
              <p class="text-xs text-content-muted">{{ t('common.status') }}</p>
            ...
            <div class="min-w-0 text-center">
              <p class="text-xs text-content-muted">{{ hasChildJobs(currentJob.id) ? t('batchImage.detail.aggregatedResult') : t('batchImage.detail.result') }}</p>
              ...
              <span class="text-gray-300 dark:text-dark-500">/</span>
              <span :class="(currentDisplayJob || currentJob).fail_count > 0 ? 'text-red-600 dark:text-red-300' : 'text-content-subtle'">{{ (currentDisplayJob || currentJob).fail_count }}</span>
            </p>
            </div>
            <div class="min-w-0 text-center">
              <p class="text-xs text-content-muted">{{ t('batchImage.detail.cost') }}</p>
              <p class="mt-1 truncate font-medium text-content-strong">{{ costLabel(currentDisplayJob || currentJob) }}</p>
            </div>
            <div class="min-w-0 text-center">
              <p class="text-xs text-content-muted">{{ t('batchImage.detail.downloadStatus') }}</p>
              <p class="mt-1 truncate font-medium text-content-strong">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          <h3 class="text-sm font-semibold text-content-strong">{{ t('batchImage.detail.items') }}</h3>
        ...
        <div v-if="items.length" class="overflow-x-auto rounded-lg border border-gray-200 bg-white dark:border-dark-700 dark:bg-dark-900">
          <table class="w-full min-w-[860px] table-fixed divide-y divide-gray-200 text-sm dark:divide-dark-700">
            ...
            <thead class="bg-surface-sunken/80">
              <tr>
                <th class="px-3 py-3 text-center text-sm font-medium text-content-muted">Custom ID</th>
                <th class="px-3 py-3 text-left text-sm font-medium text-content-muted">Prompt</th>
                <th class="px-3 py-3 text-center text-sm font-medium text-content-muted">{{ t('common.status') }}</th>
                <th class="px-3 py-3 text-center text-sm font-medium text-content-muted">{{ t('batchImage.detail.preview') }}</th>
                <th class="px-3 py-3 text-center text-sm font-medium text-content-muted">{{ t('batchImage.detail.result') }}</th>
              </tr>
            </thead>
              ...
              >
                <td class="px-3 py-2.5 text-center">
                  <span
                    ...
                    :class="isRecoveredOriginalFailure(item) ? 'text-content-subtle' : 'text-content-strong'"
                    :title="item.custom_id"
                  >
                    {{ item.custom_id }}
                  </span>
                </td>
                <td class="px-3 py-2.5 text-left" :class="isRecoveredOriginalFailure(item) ? 'text-content-subtle' : 'text-content'">
                  <div
        ...
        <div v-else class="rounded-lg border border-dashed border-gray-200 py-10 text-center dark:border-dark-700">
          ...
          <p v-if="!loadingItems" class="mt-1 text-sm text-content-muted">
      ...
      <form class="space-y-5" @submit.prevent="submitJob">
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between gap-3">
            ...
            <span class="text-xs text-content-muted">{{ t('batchImage.create.promptAdded', { count: promptRows.length }) }}</span>
          </div>
          ...
          <div v-if="promptRows.length" class="overflow-hidden rounded-lg border border-line-subtle">
            <div
            ...
            >
              <span class="w-20 flex-shrink-0 font-mono text-xs text-content-muted">{{ row.custom_id }}</span>
              <p class="min-w-0 flex-1 truncate text-sm text-content-strong">{{ row.prompt }}</p>
              <span v-if="row.output_count > 1" class="flex-shrink-0 text-xs text-content-muted">
                x{{ row.output_count }}
              </span>
              <span v-if="row.reference_images.length" class="flex-shrink-0 text-xs text-content-muted">
    ...
    <BaseDialog :show="showGuideModal" :title="t('batchImage.guide.title')" width="wide" @close="showGuideModal = false">
	      <div class="space-y-5">
	        <section class="space-y-3">
	          <h3 class="text-sm font-semibold text-content-strong">{{ t('batchImage.guide.uiTitle') }}</h3>
	        ...
	        <section class="space-y-3">
	          <div class="flex flex-wrap items-center justify-between gap-3">
	            <h3 class="text-sm font-semibold text-content-strong">{{ t('batchImage.guide.skillTitle') }}</h3>
	            <p class="text-xs text-content-muted">{{ t('batchImage.guide.skillDesc') }}</p>
	          </div>
	        <textarea
