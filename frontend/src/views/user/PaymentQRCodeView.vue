<template>
  <AppLayout>
    <div class="mx-auto flex max-w-md flex-col items-center space-y-6 py-8">
      <h2 class="text-xl font-semibold text-content-strong">
      ...
      <p v-if="qrUrl && !expired && scanHint" class="text-center text-sm text-content-muted">
        {{ scanHint }}
      </p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-content-muted">{{ qrUrl ? t('payment.qr.expiresIn') : t('payment.qr.payInNewWindowHint') }}</p>
        <p class="mt-1 text-2xl font-bold tabular-nums text-content-strong">{{ countdownDisplay }}</p>
        <p class="mt-2 text-sm text-content-subtle">{{ t('payment.qr.waitingPayment') }}</p>
      </div>
